...
class ddqn_Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        # # Get max predicted Q values (for next states) from target model (DQN)
        # Q_targets_next = self.qnetwork_target(next_states).detach().max(1)[0].unsqueeze(1)
        # # Compute Q targets for current states 
        # Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))

        # Get max predicted Q values (for next states) from target model (DDQN)
        #Find best action using local network
        Q_local_argmax = self.qnetwork_local(next_states).detach().max(1)[1].unsqueeze(1)
        #Use the action to find the value in target network
